...
export const scheduleAppointment = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
      ...
      const userAppointmentsRef = db.ref('appointments').orderByChild('bookedBy').equalTo(userId);
...
export const getAppointmentById = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
      ...
      const appointmentSnapshot = await db.ref(`appointments/${appointmentId}`).once('value');
      const appointmentData = appointmentSnapshot.val();
...
export const cancelAppointment = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const appointmentId = req.params.appointmentId;
        const userId = req.params.userId; // Assuming userId is provided in the request body
